...
class SpecialtyAdapter(private val mList: List<Specialtys>) : RecyclerView.Adapter<SpecialtyAdapter.ViewHordel>() {

    inner class ViewHordel(view: View): RecyclerView.ViewHolder(view)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SpecialtyAdapter.ViewHordel {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_specialty_and_service, parent, false)
        return ViewHordel(view)
    }

    override fun onBindViewHolder(holder: SpecialtyAdapter.ViewHordel, position: Int) {
